...
dtype *dev_dw_to_host(cudnnHandle_t handle, cudnnAttnDescriptor_t attn_desc,
    ...
    if (stride[WEIGHT_RANK - 1] != 1) {
        CUDA_CHECK(cudaMalloc((void **)&buffer, num_weights * sizeof(dtype)));
        CUDA_CHECK(cudaMemset(buffer, 0, num_weights * sizeof(dtype)));
...
dtype *host_w_to_dev(cudnnHandle_t handle, cudnnAttnDescriptor_t attn_desc,
    ...
    if (is_proj_weights && identity) {
        ...
        CUDA_CHECK(cudaMemsetAsync(weight_addr, 0, num_weights * sizeof(dtype),
                                   stream));
    } else {
...
operate(Tensor query, Tensor key, Tensor value,
    ...
    if (size_weights > 0) {
        ...
        buffers[3] = host_w_to_dev<dtype>(
            ...
            weight_desc, dev_w, o_proj.data, 1, proj_o, embed_o, false, stream);
        if (proj_bias) {
            ...
            host_w_to_dev<dtype>(handle, attn_desc, CUDNN_MH_ATTN_O_BIASES,
                                 false, size_weights, weight_desc, dev_w,
                                 o_bias->data, o_bias_len, 1, 1, false, stream);
        }
    }
    if (size_wkspace > 0) {
    ...
    if (!is_training) {
        ...
        if (size_weights > 0) {
            ...
            buffers[3] = dev_dw_to_host<dtype>(
                handle, attn_desc, CUDNN_MH_ATTN_O_WEIGHTS, true, size_weights,
                weight_desc, dev_dw, do_proj.data, 1, proj_o, embed_o, stream);
            ...
            if (proj_bias) {
                Tensor dq_bias(std::vector{q_bias_len},
                               q_bias_in ? q_bias_in->scalar_type
                                         : query.scalar_type);

                Tensor dk_bias(std::vector{k_bias_len},
                               k_bias_in ? k_bias_in->scalar_type
                                         : key.scalar_type);

                Tensor dv_bias(std::vector{v_bias_len},
                               v_bias_in ? v_bias_in->scalar_type
                                         : value.scalar_type);

                Tensor do_bias(std::vector{o_bias_len},
                               o_bias ? o_bias->scalar_type
                                      : query.scalar_type);
                if (need_to_project_input) {
                    dev_dw_to_host<dtype>(handle, attn_desc,
                                          CUDNN_MH_ATTN_Q_BIASES, false,
                                          size_weights, weight_desc, dev_dw,
                                          dq_bias.data, 1, 1, q_bias_len, stream);
                    dev_dw_to_host<dtype>(handle, attn_desc,
                                          CUDNN_MH_ATTN_K_BIASES, false,
                                          size_weights, weight_desc, dev_dw,
                                          dk_bias.data, 1, 1, k_bias_len, stream);
                    dev_dw_to_host<dtype>(handle, attn_desc,
                                          CUDNN_MH_ATTN_V_BIASES, false,
                                          size_weights, weight_desc, dev_dw,
                                          dv_bias.data, 1, 1, v_bias_len, stream);
                } else {
                ...
                dev_dw_to_host<dtype>(handle, attn_desc, CUDNN_MH_ATTN_O_BIASES,
                                      ...
                                      do_bias.data, 1, 1, o_bias_len, stream);
