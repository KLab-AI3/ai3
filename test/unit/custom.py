...
def test(mod: nn.Module, input_shape: list[int], name: str):
    ...
    try:
        if isinstance(m, mha.MHA):
            m(torch.randn(input_shape), torch.randn(input_shape), torch.randn(input_shape))
        else:
...
def main():
    ...
    test(avgpool2d.AvgPool2D(5, 2, 1, True, True, None), [3, 10, 100], 'avgpool2d')
    test(adaptiveavgpool2d.AdaptiveAvgPool2D(1), [3, 10, 100], 'adaptiveavgpool2d')
