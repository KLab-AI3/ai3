...
def run_on(*, num_samples: int, seq_len: int, embed_dim: int, num_heads: int,
    ...
    orig_out = predict_show_time(
            orig, inputs, f'pytorch \
                    num_heads: {num_heads}, bias: {bias},\
                    q: {num_samples},{seq_len},{embed_dim},\
                    k: {num_samples},{seq_len},{kdim},\
                    v: {num_samples},{seq_len},{kdim}')
    ai3.swap_mha(orig)
    ...
    compare_tensors(out, orig_out, print_pass=False, atol=1e-3)

print('Multihead Attention')
run_on(num_samples=N, seq_len=100, embed_dim=64, num_heads=4, bias=True)
print('-------------')
run_on(num_samples=N, seq_len=500, embed_dim=64, num_heads=4, bias=True)
